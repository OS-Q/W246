...
#define IP_FROM_FLASH	          3							             /*使用FLASH定义的IP信息*/
...
int write_Flash(uint32_t ADDR,uint8_t value);
//void write_MAC_to_flash(void);
/*W5500基本配置相关函数*/
void reset_w5500(void);																			/*硬复位W5500*/
void set_mac(void);																		/*配置W5500的MAC地址*/
void data_init(void);																	
void get_dhcp_ip(void);		
/*需要用定时的的应用函数*/
void timer_init(void);																	/*dhcp用到的定时器初始化*/
